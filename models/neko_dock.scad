// Нэко — скелет колонки для 3D-печати (короткий цилиндр, standalone, без дока).
// Параметрический. Подгоняй константы под свои платы.
$fn = 120;

// ============================ КОЛОНКА ============================
SPK_D     = 95;         // внешний диаметр цилиндра
H_SPK     = 60;         // высота (приземистая, как Яндекс.Станция Мини)
WALL      = 3;          // толщина стенки
BASE_T    = 6;          // толщина дна (плоское — колонка сама по себе)
TOP_T     = 3;          // толщина крышки

// OLED 0.96" SSD1306 (модуль ~27x27 мм) — экран ГОРИЗОНТАЛЬНО (лежа)
OL_W      = 13;         // окно под СТЕКЛО экрана, широкое (~13 мм по замеру, развёрнуто)
OL_H      = 10;         // окно под стекло, низкое (~10 мм по замеру)
OL_R      = 3;          // скругление окна
OL_Y      = H_SPK - 20; // центр окна от низа (прижато к верху)

FLAT_DEPTH = 8;         // насколько срезаем "лоб" цилиндра под экран/кнопки
FLAT_W     = 52;        // ширина плоского участка

// Кнопки (3 шт: VOL+, MAIN, VOL-) — колонкой под экраном
BTN_D     = 7;          // диаметр кнопочного отверстия
BTN_YS    = [10, 18, 26]; // чтоб верхняя не пересекалась с окном (низ окна ~32)
HOLE_T    = 20;         // глубина проёмов окна и кнопок (насквозь через лоб)

// Гриль — задняя половина окружности, чтобы не спорить с экраном
GRILL_H   = 18;
GRILL_W   = 4;
GRILL_A0  = 55;         // начальный угол (отсчёт от лба, 0 = фронт)
GRILL_STEP = 22.5;      // шаг по окружности
GRILL_LAST = 315;       // конечный угол (155° открыто назад, лоб чистый)

// Микрофон INMP441 — отверстия сверху кластером
MIC_R     = 6;
MIC_D     = 3;
MIC_N     = 5;

// USB-C на задней стенке (зарядка/питание, батареи нет).
// Щель под корпус разъёма: 9x4 мм по замеру, углы почти без скругления.
USB_W     = 9;          // ширина щели
USB_H     = 4;          // высота щели
USB_X     = 0.8;        // радиус скругления углов щели
USB_Z     = 25;         // центр щели от низа

// ---- ДНО-ЗАГЛУШКА: без винтов, КОНУСНАЯ посадка в чашку ----
// В нижней части корпуса — полая чашка с коническим углублением (33.5->37мм);
// дно-заглушка сужается так же (33.9->37.4), входит в углубление и клином
// расканияется. Погрешность печати ±0.4мм сдвигает глубину посадки, но не
// ломает фиксацию. Вытаскивается пальцем/отвёрткой за боковые выемки.
DECK_T    = 4;         // толщина заглушки
DECK_RB   = 33.9;      // нижний радиус заглушки (углубление тут 33.5 → клин ~0.4)
DECK_R    = 37.4;      // верхний радиус заглушки (углубление тут 37.0 → клин ~0.4)
SOCKET_H  = 7;         // глубина чашки-гнезда
SOCKET_RB = 33.5;      // радиус конического углубления снизу
SOCKET_RT = 37.0;      // радиус конического углубления сверху
BASE_FLOOR = 2;        // толщина пола чашки под углублением

face_x = SPK_D / 2 - FLAT_DEPTH;        // координата плоского "лба"
spk_in = SPK_D - 2 * WALL;              // внутренний диаметр трубки

// ------------------------- ВСПОМОГАТЕЛЬНОЕ -------------------------
// округлённый прямоугольник-«прут» вдоль оси X (толщина t)
module rod_round(w, h, r, t) {
  hull() {
    for (sy = [r, w - r])
      for (sz = [r, h - r])
        translate([0, sy - w / 2, sz - h / 2])
          rotate([0, -90, 0]) cylinder(r = r, h = t, center = true);
  }
}

// ------------------------------ КОЛОНКА ------------------------------
// Единый твёрдый объект: трубка + крышка, низ ОТКРЫТ под съёмное дно.
// Вся геометрия вырезается (никаких "плавающих" объёмов → 1 solid).
module speaker() {
  union() {
    difference() {
      // тело колонки
      cylinder(d = SPK_D, h = H_SPK);

      // D-образная полость: сзади круглая (радиус spk_in/2), спереди обрезана
      // ровно плоскостью лба face_x, снизу открыта (до z=0).
      intersection() {
        // круглое ядро полости
        translate([0, 0, -0.01])
          cylinder(d = spk_in, h = H_SPK - TOP_T + 0.02);
        // куб обрезки: по X от -spk_in/2 до face_x (плоскость лба)
        translate([-spk_in / 2, -spk_in / 2, -0.02])
          cube([spk_in / 2 + face_x, spk_in, H_SPK - TOP_T + 0.03]);
      }

      // лоб под экран и кнопки (плоский участок срезается с цилиндра).
      // Срез ограничен снизу полом BASE_T — иначе вырезается кусок дна (дырка снизу).
      translate([face_x - FLAT_DEPTH - 1, -FLAT_W / 2, BASE_T])
        cube([FLAT_DEPTH + 1, FLAT_W, H_SPK - TOP_T - BASE_T]);

      // окно OLED — канал НАСКВОЗЬ через лоб и стенку (длиннее наружного
      // диаметра), чтобы снаружи был реальный сквозной проём.
      translate([face_x + 1, 0, OL_Y])
        rod_round(OL_W, OL_H, OL_R, HOLE_T);

      // кнопки — такие же сквозные проёмы
      for (y = BTN_YS)
        translate([face_x - 1, 0, y])
          rotate([0, -90, 0]) cylinder(d = BTN_D, h = HOLE_T, center = true);

      // гриль — вертикальные щели в задней половине, сквозь стенку
      for (a = [GRILL_A0:GRILL_STEP:GRILL_LAST])
        rotate([0, 0, a])
          translate([SPK_D / 2 - WALL - 0.3, -GRILL_W / 2, BASE_T - 1])
            cube([WALL + 1.5, GRILL_W, GRILL_H]);

      // отверстия микрофона в крышке
      for (i = [0:MIC_N - 1]) {
        a = i * 360 / MIC_N;
        translate([MIC_R * cos(a), MIC_R * sin(a), H_SPK - 0.01])
          cylinder(d = MIC_D, h = TOP_T + 1);
      }
      translate([0, 0, H_SPK - 0.01]) cylinder(d = MIC_D, h = TOP_T + 1);

      // слот USB-C — на задней стенке (x=-46 центр щели, насквозь через корпус)
      translate([-SPK_D / 2 + 1.5, 0, USB_Z])
        rod_round(USB_W, USB_H, USB_X, 8);
    }

    // дно-чашка: пол 0..BASE_FLOOR + коническое углубление (куда ложится заглушка)
    cylinder(r = spk_in / 2, h = BASE_FLOOR + 0.01);
    translate([0, 0, BASE_FLOOR]) {
      difference() {
        // стенки чашки до борта корпуса
        cylinder(r = spk_in / 2, h = SOCKET_H, $fn = 120);
        // коническое углубление под заглушку
        cylinder(r1 = SOCKET_RB, r2 = SOCKET_RT, h = SOCKET_H + 0.01, $fn = 120);
        // боковые ниши: пальцу/отвёртке есть куда зацепить заглушку
        for (a = [90, 270])
          rotate([0, 0, a])
            translate([39.5, 0, SOCKET_H / 2])
              cube([3.5, 9, SOCKET_H], center = true);
      }
    }
  }
}

// --------------------------- ДНО-ЗАГЛУШКА ---------------------------
// Конусный plug: входит в коническое углубление чашки и заклинивается.
// Вынимается за боковые выемки (пальцем или плоской отвёрткой).
module deck() {
  difference() {
    cylinder(r1 = DECK_RB, r2 = DECK_R, h = DECK_T, $fn = 120);

    // выемки под палец/отвёртку у края
    for (a = [90, 270])
      rotate([0, 0, a])
        translate([DECK_RB + 2.5, 0, -0.01])
          cylinder(d = 9, h = DECK_T + 0.02, $fn = 48);
  }
}

// ------------------------------ СБОРКА ------------------------------
RENDER_DECK = 0;
if (RENDER_DECK)
  color("Gainsboro") translate([0, 0, BASE_FLOOR + DECK_T / 2]) deck();
else
  color("Gainsboro") speaker();